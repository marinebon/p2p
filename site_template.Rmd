...
params:
  site_id: "ecu-laratonera-galapagos"
  site_name:  "La Ratonera - Galapagos Islands"
editor_options:
...
if (!exists("params")) 
  params <- list(
  site_name = "ecu-laratonera-galapagos",
  site_id   = "La Ratonera - Galapagos Islands")
...
## read data from OBIS
## Molluscs
mollusc = occurrence(areaid = site$area_code, taxonid = 51)
## Ehinoderms
echino = occurrence(areaid = site$area_code, taxonid = 1806)
## Annelida
anne = occurrence(areaid = site$area_code, taxonid = 882)
## Platae
plant = occurrence(areaid = site$area_code, taxonid = 3)
...
ts_plot <- ggplot() +
  geom_histogram(data = total, aes(x = year, fill = phylum), binwidth = 3) +
...
**Mollusca**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
mytext <- paste(mollusc$scientificName, sep="") %>%
...
**Echinodermata**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
mytext2 <- paste(echino$scientificName, sep="") %>%
  lapply(htmltools::HTML)
...
**Annelida**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
mytext3 <- paste(anne$scientificName, sep="") %>%
  lapply(htmltools::HTML)
...
**Plantae**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
mytext4 <- paste(plant$scientificName, sep="") %>%
  lapply(htmltools::HTML)
