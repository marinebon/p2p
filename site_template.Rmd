...
if (is.numeric(polygon) == TRUE)
{total = occurrence(areaid = polygon, taxonid = c(51, 1806, 882, 3))
} else if(is.numeric(polygon) == FALSE){
    lat <- site$lat
    lon <- site$lon
    latmin <- lat - 2
    latmax <- lat + 2
...
## Data source:
Data shown here is extracted using the [occurrence](https://www.rdocumentation.org/packages/robis/versions/1.0.1/topics/occurrence) function of the [robis](https://www.rdocumentation.org/packages/robis/versions/1.0.1) package within polygons (e.g. EEZ, WMHS) or 2x2 degree boxes centered at monitoring site