...
```{r get_csv_data, include=F, echo=F}
...
```{r fetch_erddap_info, include=F, echo=F}
...
Map of the most recent (`r format(d,"%Y-%m-%d")`) Chlorophyll-a (CHL) around the site.
...
leaflet(
  ...
  # sst
  addWMSTiles(
    baseUrl = 'https://www.star.nesdis.noaa.gov/thredds/dodsC/CoastWatch/VIIRS/SCIENCE/chlora/Monthly2/WW00',
    layers = "nesdisVHNSQchlaMonthly:chl",
    options = WMSTileOptions(
      version = "1.3.0", format = "image/png", transparent = T, opacity = 0.7,
...
plot_timeseries(d_2, title="CHL", color="red")
