...
# define obis utility functions ----
map_obis <- function(pts, ply){
  if (nrow(pts) == 0){
    print("No data")
  } else if (nrow(pts)  > 0){
    leaflet(pts) %>%
...
ts_obis <- function(df){
  ggplot() +
  geom_histogram(data = df, aes(x = year, fill = phylum), binwidth = 3) +
...
obis_data  <- read_csv(here("data/obis/obis_data.csv"), col_types = cols()) %>% 
  st_as_sf(
...
obis_site <- obis_sites %>% 
  filter(id == params$site_id)
obis_site_data <- obis_data[st_intersects(obis_data, obis_site, sparse = F),]

obis_site_data %>% 
  st_drop_geometry() %>% 
  mutate(
    taxa = ifelse(kingdom == "Plantae", "Plantae", phylum)) %>% 
  group_by(institutionID, taxa) %>% 
  summarize(n = n())

## Plot the data ----
# title <- glue("Data source: https://obis.org/area/{site$area_code}")
ts_obis(obis_site_data %>% st_drop_geometry() %>% select(year = date_year, kingdom, phylum))
```
...
mollusc <- filter(obis_site_data, phylum == "Mollusca")
map_obis(mollusc, obis_site)
```

**Echinodermata**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
echino <- filter(obis_site_data, phylum == "Echinodermata")
map_obis(echino, obis_site)
```

**Annelida**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
anne <- filter(obis_site_data, phylum == "Annelida")
map_obis(anne, obis_site)
```

**Plantae**
```{r, echo=FALSE, message=FALSE, warning=FALSE}
plant <- filter(obis_site_data, kingdom == "Plantae")
map_obis(plant, obis_site)
```

## Data source:
Data shown here is extracted using the [occurrence](https://www.rdocumentation.org/packages/robis/versions/1.0.1/topics/occurrence){target="_blank"} function of the [robis](https://www.rdocumentation.org/packages/robis/versions/1.0.1){target="_blank"} package within polygons (e.g. EEZ, WMHS) or a 0.25 x 0.25 degrees box centered at monitoring site in data-rich regions.